class TbSalesDetail < ApplicationRecord
		...
		begin
			@@insertThread = Thread.new do
				Rails.application.executor.wrap do
					...
					Rails.logger.info "TB_SALES_DETAIL INSERT DATA CNT #{insertdata}"
					@@insertThread = nil
				end
			end
		rescue RuntimeError => runtimeerror
			Rails.logger.error "TB_SALES_DETAIL RuntimeError #{runtimeerror}"
			@@insertThread.exit
			@@insertThread = nil
		end
	...
	def self.insertdata(_datas)
		...
		if _datas.blank?
			Rails.logger.info "SALES_DETAIL CUBE DATA CNT #{cnt}"
			return cnt
		end	
		...
		begin
			...
			Rails.logger.error "TB_SALES_DETAIL Insert Error #{exception}"
			next
		end
			cnt += 1
		end	
	
		Rails.logger.info "SALES_DETAIL CUBE DATA CNT #{_datas.length} : Insert #{cnt}"
		return cnt
	end
