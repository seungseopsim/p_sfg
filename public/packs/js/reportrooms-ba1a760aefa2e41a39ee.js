  ...
  }, 3000);
  {
    ...
    if (null != buttons) {
      for (idx = 0; idx < buttons.length; ++idx) {
        ...
        if (null != frame) {
          try {
            frame.addEventListener('click', myClickHandler, function (e) {
              alert("ASDFAS");
            });
          } catch (error) {
            console.error(error);
        ...
        $(buttons[idx].dataset.target).on('show.bs.collapse', function (e) {//alert("ASDFASddd");
        });
      }
    }
  }
...
/******/ });
//# sourceMappingURL=reportrooms-ba1a760aefa2e41a39ee.js.map