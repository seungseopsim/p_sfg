  ...
  }, 3000);
  {
    ...
    if (null != buttons) {
      for (idx = 0; idx < buttons.length; ++idx) {
        frame = document.getElementById(buttons[idx].dataset.target);
        alert(buttons[idx].dataset.target);

        if (null != frame) {
          alert(frame.nodeName);
          frame.on('show.bs.collapse', function (e) {
            alert("ASDFAS");
          });
        }

        $(buttons[idx].dataset.target).on('show.bs.collapse', function (e) {
          alert("ASDFAS");
        });
      }
    }
...
/******/ });
//# sourceMappingURL=reportrooms-fdfc22d5ce65f4a9c9a4.js.map